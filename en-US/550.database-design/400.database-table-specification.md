...
A table is a structure for organizing and storing data in a database. You can design appropriate tables in accordance with the database development specifications to facilitate data management and processing and ensure data security and efficiency, thereby improving the value of data.

This topic describes the naming conventions and best practices for structure design.

## Naming conventions

When you design database tables, make sure that the table naming conventions are clear and accurate. Appropriate naming conventions can not only improve code reliability but also ensure structural unity and maintainability of data.

### Table naming conventions

* Table names must not contain special characters. We recommend that table names do not start or end with an underscore (_), or start with a digit.
* Table names must not contain reserved words or keywords.
* Table names must not contain only digits concatenated with underscores.
* Table names must be in singular forms instead of plural forms.
* We recommend that table names use the same letter case.
* Table names must have clear semantics that are easy to understand. For example, you can name a test table as `test`.
* A table name must start with the subsystem name or a general standard abbreviation and end with the functionality description. The two parts must be separated with an underscore (_), for example, `account_user`.
* If table names must be appended with a numeric identifier, we recommend that the numeric identifier increment progressively starting from 00, for example, `account_user_00`.
* Names of partitioned tables related to time must be in the format of "table name_time". The time can be an abbreviation of four to six digits, for example, `account_user_2201`.
* Intermediate result tables must be named in the format of "tmp_table name (full name or abbreviation)_column name (full name or abbreviation)_creation time", for example, `tmp_account_tbluser_20220224`.
* Backup tables must be named in the format of "bak_table name (full name or abbreviation)_column name (full name or abbreviation)_creation time", for example, `bak_account_tbluser_20220224`.

### Column naming conventions

* Column names must not contain special characters. We recommend that column names do not start or end with an underscore (_), or start with a digit.
* Column names must not contain reserved words or keywords.
* We recommend that column names do not contain only digits concatenated with underscores.
* We recommend that column names be in singular forms instead of plural forms.
* We recommend that column names use the same letter case.
* Column names must directly indicate the functionality, for example, `name`.
* A column name must contain the functionality description or a general standard abbreviation, in the format of "business name_functionality", for example, `task_num`, `create_date`, `station_desc`, and `task_id`.
...
* Prioritize business performance: You should take into consideration the business performance in table structure design, rather than simply following the three paradigms. Design data redundancy in storage to reduce table correlations and improve performance. A redundancy column cannot be:

  * A column subject to frequent modification
  * An excessively long column

* Specify a primary key: You must specify a primary key for each table. We recommend that you use a business column rather than an auto-increment column as the primary key or federated primary key.
  ...
  Tables in OceanBase Database are index-organized tables (IOTs). If you do not specify a primary key for a table, the system automatically generates a hidden primary key for the table.
    ...
    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>Tables in MySQL Database are also IOTs and tables in Oracle Database are not. For tables migrated from an Oracle database to OceanBase Database, we recommend that you use a unique index as the primary key to achieve similar storage effects as Oracle tables. For example, for the following partitioned tables and Oracle heap tables, the primary key does not need to contain the partitioning key. </p>
    </main>
...
* To use partitioned tables, you need to use appropriate partitioning keys and partitioning strategies. Only recommendations on partitioning type selection are provided here. For more information, see [Overview](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001717116).
