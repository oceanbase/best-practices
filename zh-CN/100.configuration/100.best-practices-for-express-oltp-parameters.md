...
## OceanBase 数据库 V3.x 版本 EXPRESS_OLTP 场景参数配置最佳实践

### OceanBase 数据库 V3.x 版本 EXPRESS_OLTP 场景配置项推荐配置
...
### OceanBase 数据库 V3.x 版本 EXPRESS_OLTP 场景配置项配置示例
...
### OceanBase 数据库 V3.x 版本 EXPRESS_OLTP 场景变量推荐配置
...
|  ob_sql_work_area_percentage |  指定 SQL 工作区的内存占整个租户内存的百分比。 ob_sql_work_area_percentage 是 SQL 工作区内存参数，表示 SQL 可能耗费大量内存的SQL排序等阻塞性算子所使用的内存。  |  5  |  [0, 100]  |  5。 <main id="notice" type='explain'><h4>说明</h4><p>租户内存太小时可以适当调大 （比如调大至 20）。只读或 AP 场景也可适当调大。</p></main>  | GLOBAL |
...
### OceanBase 数据库 V3.x 版本 EXPRESS_OLTP 场景变量配置示例
