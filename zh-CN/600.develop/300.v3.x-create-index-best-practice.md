...
在 V4.x 版本中，全局索引和局部索引都是基于整张表的数据构建，因此它们的构建所需的临时空间与整张表的数据量密切相关。
...
我们有两种方式来估算：第一种是利用表的统计信息，如果可用；第二种是通过其他内部表进行简单估算。
...
<main id="notice" type='explain'>
  ...
  <p>
    <ul>
      <li>计算临时空间后，还需考虑每台 ObServer 默认预留 10% 的空间给其他任务。</li>
      <li>在公有云中，可能具备磁盘自动扩容的能力，当前步长为 50G。由于建索引的写入速度较快，扩容速度可能无法跟上，导致磁盘空间不足。因此，建议提前估算空间并一次性扩容。</li>
    </ul>
  </p>
</main>
...
<main id="notice" type='explain'>
  <h4>注意</h4>
  <p>删除列操作是借助 DAG 来调度完成的，因此在调整上述参数的同时，需要调整 DAG 的并行度：`ALTER SYSTEM SET ddl_thread_score = 8 tenant = xxx;` （该参数在 V4.1 和 V4.2.x 中提供）。</p>
</main>
...
如果您使用的是 OceanBase V4.2.1 版本，已知上述模块占用内存较多；V4.2.1 及更高版本如果需要获取更好性能，建议也检查上述模块的内容占用情况。
